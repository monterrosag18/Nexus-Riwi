...
export default function createFactionChat() {
    ...
    function renderMessages(clanId) {
        const messages = store.getChatMessages(clanId);
    ...
    function attachEvents() {
        ...
        if (input && sendBtn) {
            const sendMessage = () => {
                ...
                if (text && user) {
                    store.addChatMessage(user.clan, user, text);
                    input.value = '';
                    // Re-render handled by store subscription normally, 
                    // but we'll manually refresh if needed or just let subscription do it.
                }
            };
    ...
    store.subscribe(() => {
        if (activeTab === 'clan') {
            ...
            if (history) {
                const user = store.getState().currentUser;
                if (user) {
                    history.innerHTML = renderMessages(user.clan);
